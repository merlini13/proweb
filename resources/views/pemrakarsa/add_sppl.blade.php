...
<div class="row-fluid sortable">
  <div class="box span12">
    <div class="box-header" data-original-title>
      <h2><i class="halflings-icon edit"></i><span class="break"></span>Form Buat Perizinan SPPL</h2>
    </div>
    <div class="box-content">
      <form class="form-horizontal" action="{{url('/save-data-sppl')}}" method="POST" enctype="multipart/form-data">
        {{csrf_field()}}
        <fieldset>
          ...
          <div class="control-group">
            ...
            <div class="controls">
            <input type="text" class="input-xlarge" name="nama_usaha" required="">
            </div>
          </div>
          ...
          <div class="control-group">
            ...
            <div class="controls">
            <input type="text" class="input-xlarge" name="alamat_usaha" required="">
            </div>
          </div>
          ...
          <div class="controls">
            <select id="selectError3" name="matriks_id">
            <option>Pilih Jenis Usaha</option>
            <?php
            $all_published_izin=DB::table('matriks')
                                  ->get();
            foreach($all_published_izin as $izin){?>
            <option value="{{$izin->matriks_id}}">{{$izin->jenis_usaha}}</option>
          <?php } ?>
            </select>
          </div>
          </div>
